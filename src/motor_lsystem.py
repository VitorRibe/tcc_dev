...
class MotorLSystem:
    def __init__(self, caminho_dll: str = None):
        ...
        self.lib.expandir_lsystem_c.argtypes = [ctypes.c_char_p, ctypes.POINTER(ctypes.c_char_p), ctypes.c_int]
    ...
    def gerar(self, axioma: str, regras: dict, iteracoes: int) -> str:
        ...
        for i in range(256): regras_c[i] = None
        for k, v in regras.items(): regras_c[ord(k)] = v.encode('utf-8')

        ponteiro_resultado = self.lib.expandir_lsystem_c(axioma_bytes, regras_c, iteracoes)
        if not ponteiro_resultado: raise MemoryError("Falha de alocação no motor nativo (C).")
